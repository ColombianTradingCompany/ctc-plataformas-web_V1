...
| consolas | kaffetal-regal, cherry-picked, herramientas-internas | **La oferta en dos caminos** (`PVC_BCP_PLAN.md` §9.5): Cherry Picked = PVC × mult(grado); compra directa sin adscripción = PVC × mult(grado) × (1 − prima 8 %) con `lot_offers` kind `directa`; la **herramienta «PVC × grado»** (edición vigente × Punto y Tríada → oferta) para OCP, KR y campo. Auditoría del módulo en §10 (A1–A12) | 2026-09-15 |
| owner | consolas | Confirmar (§10.3): que el ciclo semanal **informa** (reporte + contenido) y el precio sigue publicándose por franja; que la vista pública deje de exponer el desglose de costos (A7); cómo se retira el 8 % en compra directa y qué es «cantidad mínima» (§9.5) | 2026-09-15 |
