...
| consolas (OCP · Contratos, Fincas, Nominados) | kaffetal-regal | **El trato real contradice §12.9**: `RELEASE_STAIRCASE` en `contractActions.ts` libera 50/75/100 % sin penalización (y KR lo publica así en «El trato»), mientras §12.9 manda 0/25/50 % + 4 % (`compromiso.ts`); y **pasos que no avisan al productor**: finca aprobada o rechazada (`approveFinca`/`rejectFinca`), muestra recibida, entrada al bache, firma del contrato. Los dos bloquean el video del productor (`briefs/kaffetal-regal-guion-video-productor.md` §4 y §6) | 2026-09-16 |
