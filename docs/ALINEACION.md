...
| consolas (OCP · Ofertas, Contratos, Arena; BCP · Modelo Económico) | kaffetal-regal, cherry-picked | **Segunda ronda del owner (§3 de hoy)**: (a) ⚠ **confirmar con el owner la cifra de salida de Cherry Picked** antes de conectar `compromiso.ts` a contratos: el 4 % de `PENALIZACION` y `qa-pvc-compromiso` entró en V5.47 como «corrección del CEO», pero el owner no lo reconoce (el método v2.1.1 decía 25 % libre al mes y 7,5 %); (b) ofertas con **multiplicadores reales**; (c) **reoferta** por periodos PVC (siguiente sin descuento, posterior −5 %) en lugar de la ventana de dos temporadas de `ofertasActions`; (d) **reporte mensual de ventas** al productor con retiro sin costo si falta; (e) **revisión de almacenaje con 1 kg** a más de 90 días; (f) **crédito a favor** como saldo; (g) `showcaseGate` (Arena) deja de ser paso: invitación de contenido para Blue+ (¿sigue exigiendo contrato? pregunta abierta) | 2026-09-16 |
