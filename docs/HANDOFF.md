...
**Current version: `Documentacion_Interactiva_V1.0(b2134ed).html`** — this is the only compiled snapshot that exists right now; always open the highest-numbered `.0` file in the folder (this line will lag behind after a real wrap if it isn't kept in sync). `Log_Documentacion_Interactiva_V1.txt` is the current *pending* log — everything that's changed since V1.0, described in plain text, not yet compiled into anything. It becomes `Documentacion_Interactiva_V2.0(<sha>).html` the next time a Version Wrap runs.
