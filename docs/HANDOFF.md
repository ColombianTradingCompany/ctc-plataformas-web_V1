...
**Current version: `Documentacion_Interactiva_V2.0(8e96527).html`** — always open the highest-numbered `.0` file in the folder (this line will lag behind after a real wrap if it isn't kept in sync). `Documentacion_Interactiva_V1.0(b2134ed).html` remains archived alongside it (never deleted) as the previous snapshot. `Log_Documentacion_Interactiva_V2.txt` is the current *pending* log — everything that's changed since V2.0, described in plain text, not yet compiled into anything; `Log_Documentacion_Interactiva_V1.txt` is closed and kept as the historical record of what went into producing V2.0. The next Version Wrap produces `Documentacion_Interactiva_V3.0(<sha>).html`.
