...
- **Kaffetal Club (2026-07-14)**: `club_member_codes` (BCP-emitted member codes: `code` unique `KC-XXXX-XXXX`, `note`, `created_by`, `redeemed_by`/`redeemed_at`, `revoked_at`; both profile FKs are `ON DELETE SET NULL` **on purpose** — the first QA teardown proved a plain FK blocks deleting any profile that ever redeemed a code) — **service-role-only by design** (RLS enabled, zero policies, like Arena/leads). Membership itself is `producer_profiles.club_member_since` (null = not a member), guard-protected; set only via the producer redemption action (`src/lib/club/actions.ts`, which also writes an `audit_log` row and a welcome note into `producer_comm_log`) and cleared via BCP's `revokeClubMembership`. **Being a member gates**: producer-side "Mis contratos" module (locked tile + in-module code-redemption form in `AppDashboard.tsx`), BCP `signContract` (refuses non-member producers), and BCP `publishLot` (defense in depth — non-member lots can't enter the Cherry Picked catalog). Contracts are still auto-created at grading for everyone; it's *signing* that requires membership, so a producer who joins later finds their pending contract waiting. BCP emits/revokes codes and removes members in `/bcp/club` (`clubActions.ts`).
...
**CTC Home** — marketing site + lead capture (2026-07-13): the "Escríbenos"/service contact modals no longer open a mailto — they create a lead + platform account (Google via the root `/auth/callback`, or created on the visitor's behalf with a temp password; Co-Create → Cherry Picked buyer, everything else → Kaffetal Regal producer via the shared `src/lib/auth/promoteFreshBuyerToProducer.ts`) and send a welcome email. Leads are triaged in /bcp/leads.

**Kaffetal Regal** — real auth (email/password + Google), real finca/lot CRUD, full Ficha Técnica (all 8 panes ported 1:1 from the source CTC datasheet tool), dashboard with per-lot sparkline/kanban/AI-advice display. **Kaffetal Club (2026-07-14)**: the "Mis contratos" hub module is member-only — non-members see a locked tile and an in-module explanation + code-redemption form (see the Kaffetal Club schema bullet for the full mechanics). This is the most actively developed surface right now. **UI/UX pass 2026-07-10** (first of an ongoing progressive pass across all platforms) reworked the producer profile and the whole Ficha flow:
...
**Current version: `Documentacion_Interactiva_V3.0(11268a0).html`** — always open the highest-numbered `.0` file in the folder (this line will lag behind after a real wrap if it isn't kept in sync). `V2.0(8e96527)` and `V1.0(b2134ed)` remain archived alongside it (never deleted) as previous snapshots. `Log_Documentacion_Interactiva_V3.txt` is the current *pending* log (empty at wrap time — everything since V3.0 goes here); `Log_..._V2.txt` and `V1.txt` are closed historical records of what went into producing V3.0 and V2.0 respectively. V3.0 added the whole **Leads CTC Home** subsystem (leads/lead_replies + the `/bcp/leads` triage board + lead-email flow), the EUDR **certificates** (finca dossier + lot certificate, producer-facing), **derived EUDR risk factors** + the producer-answer/CTC-evaluated split, the **producer module hub**, the BCP **Panel triage**, and two new guard triggers. The next Version Wrap produces `Documentacion_Interactiva_V4.0(<sha>).html`.
