...
> **⚠ V5.17 (2026-08-21) — el escritor del grado CAMBIÓ.** La evaluación por **Q-Grader en bache**
> (la maquinaria de sondeo de Nominados) es ahora EL camino del galardón: `recordEvaluationVerdict`
> (ex-`recordSondeoResult`, `nominadosActions.ts`) deriva el grado del puntaje con `gradoPorPuntaje()`
> («el puntaje manda»), inserta la planilla como `lot_evaluations` con **`source='q_grader_batch'`**
> (firmada por `sondeo_batches.q_grader_name`), escribe `grade` + `stage='galardonado'`, deja la
> inscripción en la fase terminal nueva **`galardonado`** y **otorga la membresía del Club**
> (`grantClubMembershipOnce`, `src/lib/arena/club.ts`). La escalera del productor es
> FT·FT2·EUDR·VID → EVA → MUE·SON → **GAL** → ARE (la Arena queda como **vitrina opcional
> post-galardón**, re-gate pendiente en V5.19). Ningún veredicto nuevo escribe `phase='arena'`, así
> que ningún lote nuevo llega a una sesión hasta el re-gate. Las descripciones de abajo cuentan el
> modelo 2026-07-17/20 y siguen siendo exactas para la MAQUINARIA (baches, fases, pagos); donde
> digan que la jornada grada o que el Club llega al competir, manda este banner. Barrido completo
> del documento: V5.19.
