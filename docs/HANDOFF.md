...
- **Kaffetal Regal**: `fincas` (+ `video_asset_id`, added 2026-07-10), `lots` (stage enum borrador→...→galardonado, `datasheet` jsonb holding the full ~50-field Ficha, `ai_next_step_advice`/`ai_next_step_context` for the AI-advice cache, `video_asset_id` — pre-existing column, only wired to real UI as of 2026-07-10; `sample_shipped_at` added 2026-07-10, the producer half of the shipment handoff — see BCP section), `ficha_completion_snapshots`, `media_assets`.
...
**As of 2026-07-10 this is no longer a single always-live file** — it's versioned snapshots, managed by the `architecture-doc-versioning` skill (`.claude/skills/architecture-doc-versioning/` at the repo root, one level up from `ctc-platform/`). Read that skill before touching anything in `docs/architecture/`. Short version: don't edit the HTML directly mid-session anymore — log what changed to the current `Log_Documentacion_Interactiva_V{N}.txt`, and only compile it into a new `Documentacion_Interactiva_V{major}.{minor}(<commitSha>).html` snapshot during an explicit "Version Wrap" (user-requested or Claude-suggested). Every snapshot and log ever produced stays in the folder — nothing gets deleted, so the whole history is browsable by filename.

**Current version: `Documentacion_Interactiva_V1.1(880f8c8).html`** — always open the highest-numbered file in the folder, this line will lag behind after future wraps if it isn't kept in sync. `Documentacion_Interactiva_V1.0(b2134ed).html` is the original snapshot, kept for history. `Log_Documentacion_Interactiva_V1.txt` (wrapped, historical) covers everything between those two; `Log_Documentacion_Interactiva_V2.txt` (if present) is the current pending log.
