...
La conversación la lleva CTC desde el BCP.
...
| `/bcp/ctc-tech` | el tablero de leads del pilar `tech` (kanban por estado, respuesta con correo, aprovisionamiento) |
...
- `src/app/bcp/(app)/ctc-tech/` + `src/components/panel/{LeadsBoard.tsx,leadsActions.ts}` (compartidos con la LCP) +
...
**BCP · CTC Tech / Leads** lo es todo aquí: leer, responder (con la contraseña temporal la primera vez),
