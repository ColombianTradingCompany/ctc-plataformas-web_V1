...
// ── 6. Los Baches de Evaluación: abierto → en_centro → cerrado ──────────────
{
  ...
  check("exige el Q-Grader (firma la planilla oficial; se teclea hasta la fase 4)", env.includes("if (!qGrader) return"));
  check("solo un bache abierto y con lotes", env.includes('batch.status !== "abierto"') && env.includes("if (!lotes.length) return"));
  check("deja el bache en el Centro, con su credencial si hay una", env.includes('status: "en_centro"') && env.includes("centro_calidad_account_id"));
