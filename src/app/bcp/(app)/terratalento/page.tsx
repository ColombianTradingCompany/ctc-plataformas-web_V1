...
// ── Terratalento · el tablero de match (V2; BCP desde la V5.60) ─────────────────────────
      ...
      <section style={{ marginTop: 34 }}>
        ...
        {recolectores.length > 0 && (
          <div className={styles.tabs} style={{ flexWrap: "wrap" }}>
            <Link href="/bcp/terratalento" className={!mun && disp !== "1" ? styles.tabActive : undefined}>Todos</Link>
            <Link href="/bcp/terratalento?disp=1" className={disp === "1" && !mun ? styles.tabActive : undefined}>Solo disponibles</Link>
            {municipios.map((m) => (
              <Link key={m} href={`/bcp/terratalento?mun=${encodeURIComponent(m)}`} className={mun === m ? styles.tabActive : undefined}>
                {m}
              </Link>
            ))}
          </div>
