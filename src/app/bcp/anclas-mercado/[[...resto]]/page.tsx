...
// ── Talón de la mudanza: `/bcp/anclas-mercado` ──
...
export default async function TalonBcpAnclasMercado({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/bcp/anclas-mercado" + cola) ?? "/ecp");
}
