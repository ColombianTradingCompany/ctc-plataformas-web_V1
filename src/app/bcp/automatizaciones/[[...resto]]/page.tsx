...
// ── Talón de la mudanza: `/bcp/automatizaciones` ──
...
export default async function TalonBcpAutomatizaciones({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/bcp/automatizaciones" + cola) ?? "/ecp");
}
