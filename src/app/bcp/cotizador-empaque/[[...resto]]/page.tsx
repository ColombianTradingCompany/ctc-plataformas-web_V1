...
// ── Talón de la mudanza: `/bcp/cotizador-empaque` ──
...
export default async function TalonBcpCotizadorEmpaque({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/bcp/cotizador-empaque" + cola) ?? "/ecp");
}
