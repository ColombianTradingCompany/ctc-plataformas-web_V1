...
// ── Talón de la mudanza: `/bcp/cotizador-logistico` ──
...
export default async function TalonBcpCotizadorLogistico({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/bcp/cotizador-logistico" + cola) ?? "/ecp");
}
