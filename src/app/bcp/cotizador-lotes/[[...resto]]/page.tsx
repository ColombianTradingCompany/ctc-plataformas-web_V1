...
// ── Talón de la mudanza: `/bcp/cotizador-lotes` ──
...
export default async function TalonBcpCotizadorLotes({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/bcp/cotizador-lotes" + cola) ?? "/ecp");
}
