...
// ── Talón de la mudanza: `/bcp/direccionamiento` ──
...
export default async function TalonBcpDireccionamiento({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/bcp/direccionamiento" + cola) ?? "/ecp");
}
