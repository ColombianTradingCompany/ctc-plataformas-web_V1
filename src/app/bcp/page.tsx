...
export default async function BcpDashboardPage() {
  ...
  const { data: profile } = await session.from("profiles").select("role, full_name").eq("id", user.id).single();
  ...
  return (
    <div className={styles.wrap}>
      ...
      <section>
        <h2>Fincas pendientes de revisión</h2>
