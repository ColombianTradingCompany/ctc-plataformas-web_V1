...
// ── ECP · Herramientas Internas · Cotizadores · Cuadro de evaluación del costo de empaque ────────────
...
import { QUOTE_BASE_PATH, QUOTE_STATUS_LABEL, type QuoteStatus } from "@/lib/cotizador/types";
  ...
  if (!rows.length) {
    return (
      <div className={styles.empty}>
        ...
        <Link className={styles.backLink} href={QUOTE_BASE_PATH.empaque}>← Ir al cotizador</Link>
      </div>
    );
  }
        ...
        <div className={ev.pickList}>
          {usable.map((r) => (
            <label key={r.id} className={ev.pick}>
              ...
              <Link className={ev.pickMeta} href={`${QUOTE_BASE_PATH.empaque}/${r.id}`}>abrir →</Link>
            </label>
          ))}
        </div>
            ...
            {sorted.map((r, i) => {
              ...
              return (
                <div key={r.id} className={ev.row}>
                  <Link className={ev.rowName} href={`${QUOTE_BASE_PATH.empaque}/${r.id}`}>
                    {r.title}
          ...
          <table className={ev.table}>
            ...
            <tbody>
              {sorted.map((r) => (
                <tr key={r.id}>
                  <td>
                    <Link href={`${QUOTE_BASE_PATH.empaque}/${r.id}`}>{r.title}</Link>
                    <span className={ev.rowCode}>{r.code}</span>
                  </td>
