...
export const metadata: Metadata = { title: "PVC · Dossier · ECP", robots: { index: false, follow: false } };
              ...
              {docs.filter((d) => d.version === v).map((d) => (
                <tr key={d.file}>
                  ...
                  <td style={{ textAlign: "right" }}>
                    <a className="btn btn-sm" href={`/ecp/pvc/dossier/${encodeURIComponent(v)}/${encodeURIComponent(d.file)}`} target="_blank" rel="noopener">{d.kind === "pdf" ? "Abrir" : "Descargar"}</a>
                  </td>
                </tr>
              ))}
            </tbody>
