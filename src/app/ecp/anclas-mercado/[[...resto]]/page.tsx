...
// ── Talón de la mudanza: `/ecp/anclas-mercado` ──
...
export default async function TalonEcpAnclasMercado({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/ecp/anclas-mercado" + cola) ?? "/bcp");
}
