...
// ── Talón de la mudanza: `/ecp/cotizador-empaque` ──
...
export default async function TalonEcpCotizadorEmpaque({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/ecp/cotizador-empaque" + cola) ?? "/bcp");
}
