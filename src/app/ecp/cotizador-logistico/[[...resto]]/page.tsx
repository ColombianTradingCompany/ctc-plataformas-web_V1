...
// ── Talón de la mudanza: `/ecp/cotizador-logistico` ──
...
export default async function TalonEcpCotizadorLogistico({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/ecp/cotizador-logistico" + cola) ?? "/bcp");
}
