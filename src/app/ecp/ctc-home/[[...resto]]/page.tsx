...
// ── Talón de la mudanza: `/ecp/ctc-home` ──
// V5.59 (2026-09-19): nace la LCP, la cuarta consola, y se lleva lo que entra de fuera —
// la lista de espera de CTC Home pasa a ser `/lcp/lista-espera`, que reúne todas las listas (docs/OVERHAUL_CONSOLAS_PLAN.md, fase 1).
...
export default async function TalonEcpCtcHome({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/ecp/ctc-home" + cola) ?? "/lcp");
}
