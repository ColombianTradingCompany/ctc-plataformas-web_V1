...
// ── Talón de la mudanza: `/ecp/ctc-tech` ──
...
export default async function TalonEcpCtcTech({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/ecp/ctc-tech" + cola) ?? "/bcp");
}
