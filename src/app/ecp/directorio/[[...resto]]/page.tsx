...
// ── Talón de la mudanza: `/ecp/directorio` ──
...
export default async function TalonEcpDirectorio({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/ecp/directorio" + cola) ?? "/bcp");
}
