...
// ── Talón de la mudanza: `/ecp/documentacion` ──
...
export default async function TalonEcpDocumentacion({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/ecp/documentacion" + cola) ?? "/bcp");
}
