...
// ── Talón de la mudanza: `/ecp/grados` ──
...
export default async function TalonEcpGrados({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/ecp/grados" + cola) ?? "/bcp");
}
