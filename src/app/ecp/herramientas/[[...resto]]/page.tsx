...
// ── Talón de la mudanza: `/ecp/herramientas` ──
// V5.60 (2026-09-19): el nuevo reparto BCP ↔ ECP — fase 2 de docs/OVERHAUL_CONSOLAS_PLAN.md.
// Este módulo pasa del ECP a «BCP · Ecosistema de Valor».
...
export default async function TalonEcpHerramientas({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/ecp/herramientas" + cola) ?? "/bcp");
}
