...
// ── Talón de la mudanza: `/ecp/leads` ──
// V5.59 (2026-09-19): nace la LCP, la cuarta consola, y se lleva lo que entra de fuera —
// «Leads · Recepción» deja el ECP (docs/OVERHAUL_CONSOLAS_PLAN.md, fase 1).
...
export default async function TalonEcpLeads({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/ecp/leads" + cola) ?? "/lcp");
}
