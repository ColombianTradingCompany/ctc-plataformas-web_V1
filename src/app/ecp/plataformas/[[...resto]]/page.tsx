...
// ── Talón de la mudanza: `/ecp/plataformas` ──
// V5.60 (2026-09-19): el nuevo reparto BCP ↔ ECP — fase 2 de docs/OVERHAUL_CONSOLAS_PLAN.md.
// Este módulo pasa del ECP a «BCP · Configuración del Sistema».
...
export default async function TalonEcpPlataformas({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/ecp/plataformas" + cola) ?? "/bcp");
}
