...
// ── Talón de la mudanza: `/ecp/terratalento` ──
...
export default async function TalonEcpTerratalento({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/ecp/terratalento" + cola) ?? "/bcp");
}
