...
// ── Talón de la mudanza: `/ecp/usuarios` ──
...
export default async function TalonEcpUsuarios({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/ecp/usuarios" + cola) ?? "/bcp");
}
