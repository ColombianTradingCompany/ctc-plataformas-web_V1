...
// ── Talón de la mudanza: `/ecp/varietales` ──
...
export default async function TalonEcpVarietales({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/ecp/varietales" + cola) ?? "/bcp");
}
