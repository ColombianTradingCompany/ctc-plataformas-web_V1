...
// La jornada en vivo sigue viviendo en /ocp/arena/[sessionId]/run.
...
export default async function BcpArenaPage() {
  ...
  const sessionHead = (s: SessionRow) => (
    <div style={{ display: "flex", alignItems: "baseline", gap: 8, flexWrap: "wrap" }}>
      <Link href={`/ocp/arena/${s.id}`} style={{ textDecoration: "none", color: "inherit" }}>
        <b style={{ fontSize: 14 }}>
          {seasonOf(s)} · {dateOf(s)}
        </b>
      </Link>
        ...
        return (
          <div key={s.id} className={styles.miniCard}>
            ...
            <p className={styles.meta} style={{ margin: "4px 0 6px" }}>
              {jornadaOwns ? (
                <>
                  Jornada en curso — <Link href={`/ocp/arena/${s.id}/run`}>abrir el runner →</Link>
                </>
              ) : (
            ...
            {!jornadaOwns && (
              <div style={{ display: "grid", gap: 6 }}>
                ...
                <Link href={`/ocp/arena/${s.id}`} className={styles.meta} style={{ textDecoration: "underline" }}>
      ...
      body: culminadas.map((s) => (
        <div key={s.id} className={styles.miniCard}>
          ...
          <p className={styles.meta} style={{ margin: "6px 0 0" }}>
            ...
            <Link href={`/ocp/arena/${s.id}`}>Ver sesión →</Link>
          </p>
        </div>
      )),
    },
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
        ...
        <Link href="/ocp/arena/temporadas" className={styles.backLink}>
          Temporadas →
        </Link>
      </div>
