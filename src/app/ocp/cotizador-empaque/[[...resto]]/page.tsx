...
// ── Talón de la mudanza: `/ocp/cotizador-empaque` ──
...
export default async function TalonOcpCotizadorEmpaque({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/ocp/cotizador-empaque" + cola) ?? "/ecp");
}
