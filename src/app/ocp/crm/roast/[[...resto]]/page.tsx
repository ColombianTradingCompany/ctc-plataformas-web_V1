...
// ── Talón de la mudanza: `/ocp/crm/roast` ──
// V5.59 (2026-09-19): nace la LCP, la cuarta consola, y se lleva lo que entra de fuera —
// el CRM CP Roast deja el OCP (docs/OVERHAUL_CONSOLAS_PLAN.md, fase 1).
...
export default async function TalonOcpCrmRoast({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/ocp/crm/roast" + cola) ?? "/lcp");
}
