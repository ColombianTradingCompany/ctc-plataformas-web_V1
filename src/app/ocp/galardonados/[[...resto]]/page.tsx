...
// ── Talón de la mudanza: `/ocp/galardonados` ──
// V5.61 (2026-09-19): Productores, Fincas y Lotes se funden en UNA tabla, `/ocp/kr` — fase 3 de
// docs/OVERHAUL_CONSOLAS_PLAN.md. Galardonados dejó de ser módulo: es el filtro «Galardonados» de la tabla.
...
export default async function TalonOcpGalardonados({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/ocp/galardonados" + cola) ?? "/ocp");
}
