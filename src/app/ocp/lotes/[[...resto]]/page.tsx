...
// ── Talón de la mudanza: `/ocp/lotes` ──
// V5.61 (2026-09-19): Productores, Fincas y Lotes se funden en UNA tabla, `/ocp/kr` — fase 3 de
// docs/OVERHAUL_CONSOLAS_PLAN.md. La tarjeta de cada lote es `?lote=<id>`.
...
export default async function TalonOcpLotes({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/ocp/lotes" + cola) ?? "/ocp");
}
