...
// ── Talón de la mudanza: `/ocp/productores` ──
// V5.61 (2026-09-19): Productores, Fincas y Lotes se funden en UNA tabla, `/ocp/kr` — fase 3 de
// docs/OVERHAUL_CONSOLAS_PLAN.md. El panel de cada productor es ahora `?productor=<id>`.
...
export default async function TalonOcpProductores({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/ocp/productores" + cola) ?? "/ocp");
}
