...
// ── Talón de la mudanza: `/ocp/socios` ──
...
export default async function TalonOcpSocios({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/ocp/socios" + cola) ?? "/bcp");
}
