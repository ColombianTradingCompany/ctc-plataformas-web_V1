...
// ── Talón de la mudanza: `/ocp/transcripciones` ──
...
export default async function TalonOcpTranscripciones({ params }: { params: Promise<{ resto?: string[] }> }) {
  const { resto } = await params;
  const cola = resto?.length ? "/" + resto.join("/") : "";
  permanentRedirect(destinoDe("/ocp/transcripciones" + cola) ?? "/ecp");
}
