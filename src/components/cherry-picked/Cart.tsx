...
import { CUR, importe, fmt, type CartSummary } from "./data";
...
const EN = {
  ...
  preLine: (today: string, later: string, arrival: string) => `Pre-order · today ${today} ${CUR} · balance ${later} ${CUR} on arrival (${arrival})`,
  packMain: "Sample pack · main harvest",
  packSub: `28–35 × 125 g · 300 ${CUR} · flies October 2026`,
...
const T: Record<Lang, typeof EN> = {
  ...
  es: {
    ...
    preLine: (today: string, later: string, arrival: string) => `Preorden · hoy ${today} ${CUR} · saldo ${later} ${CUR} al arribo (${arrival})`,
    packMain: "Pack de muestras · cosecha principal",
    packSub: `28–35 × 125 g · 300 ${CUR} · vuela en octubre 2026`,
  ...
  de: {
    ...
    preLine: (today: string, later: string, arrival: string) => `Vorbestellung · heute ${today} ${CUR} · Rest ${later} ${CUR} bei Ankunft (${arrival})`,
    packMain: "Musterpaket · Haupternte",
    packSub: `28–35 × 125 g · 300 ${CUR} · fliegt im Oktober 2026`,
...
export function Cart({
  ...
  const shipLbl =
    shipZone === "EXW"
      ? t.shipExw
      : `${t.lastMile} · ${shipZone} · ${importe(summary.rate, lang)} ${CUR}/kg × ${fmt(summary.kg, lang)} kg`;
      ...
      <div className={styles.cartBody}>
          ...
          <div className={styles.cartLine} key={i.id}>
            <div>
              ...
              <div className={styles.clSub}>
                {fmt(i.kg, lang)} kg · {fmt(Math.round(i.total), lang)} {CUR}<br />
                {i.mode === "spot"
                  ? t.spotLine
        ...
        <div className={styles.cartZone}>
          ...
          <select id="shipSel" value={shipZone} onChange={(e) => onSetZone(e.target.value)}>
            {zones.map((z) => (
              <option key={z.code} value={z.code}>{z.label} · {importe(z.ratePerKg, lang)} {CUR}/kg</option>
            ))}
          </select>
        </div>
        <div className={styles.cartTot}><span className={styles.tk}>{shipLbl}</span><span>{fmt(Math.round(summary.ship), lang)} {CUR}</span></div>
        <div className={styles.cartTot} style={{ marginTop: 4 }}>
          <span className={styles.tk}>{t.dueToday}</span>
          <span>{fmt(Math.round(summary.today), lang)} {CUR}</span>
        </div>
        <div className={styles.cartTot}>
          <span className={styles.tk}>{t.balance(t.arrival)}</span>
          <span>{fmt(Math.round(summary.later), lang)} {CUR}</span>
        </div>
        <div className={`${styles.cartTot} ${styles.big}`}><span>{t.total}</span><span>{fmt(Math.round(summary.total), lang)} {CUR}</span></div>
