...
export async function LeadsBoard({
  ...
  const boards = pillars.map((key) => ({ key, ...PILLAR_DEFS[key] }));
  // Con un solo pilar el KPI por-pilar es redundante con el tablero de abajo.
  const showPillarKpis = boards.length > 1;
      ...
      <div className={styles.kpiGrid}>
        ...
        {!showPillarKpis && (
          <div className={styles.kpiCard}>
            <span className={styles.kpiTop}>
              <span className={styles.kpiK}>{boards[0]?.label ?? "Leads"}</span>
              <span className={styles.kpiIcon}>{boards[0]?.icon}</span>
            </span>
            <span className={styles.kpiV}>{leads.length}</span>
            <span className={styles.kpiSub}>{leads.filter((l) => l.status !== "cerrado").length} activos</span>
          </div>
        )}
        <div className={styles.kpiCard}>
          <span className={styles.kpiTop}>
      ...
      {boards.map((p) => {
        ...
        return (
          <section key={p.key} id={`board-${p.key}`} style={{ marginTop: 30, scrollMarginTop: 12 }}>
            ...
            {pillarLeads.length === 0 && <p className={styles.empty}>Sin leads de {p.label} todavía.</p>}
