...
import { setTaskState } from "@/components/panel/tareasActions";
...
export function PanelTasks({ items }: { items: PanelTaskItem[] }) {
  // Estado local optimista para que la casilla responda al instante; la acción revalida además el
  // Panel de cada consola que enseña esa tarea, así que recargar dice lo mismo.
  ...
  function toggle(key: string) {
    ...
    startTransition(async () => {
      // La acción DEVUELVE el rechazo (no lanza): hasta la V5.59 se ignoraba, y a quien no tenía permiso
      // la casilla se le quedaba marcada en pantalla y desmarcada en la base.
      const deshacer = () => setStates((s) => ({ ...s, [key]: next === "done" ? "tbd" : "done" }));
      try {
        const res = await setTaskState(key, next);
        if (!res.ok) {
          deshacer();
          setAviso(res.error);
        } else setAviso(null);
        ...
        deshacer();
        setAviso("No se pudo guardar. Inténtelo de nuevo.");
      }
    });
  }
