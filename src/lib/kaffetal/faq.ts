// ── Las 12 preguntas frecuentes de Kaffetal Regal ────────────────────────────
// Vivían dentro de `FaqSection.tsx`. Salieron a un módulo PURO el 2026-08-14
// porque ahora tienen DOS lectores: la sección que las pinta y el constructor
// de datos estructurados (`src/lib/seo/jsonLd.ts`), que las publica como
// `FAQPage` de schema.org para que Google y los modelos de lenguaje puedan
// citarlas. Duplicarlas habría garantizado que la respuesta publicada y la
// respuesta visible se separaran con el primer cambio de cifra.
//
// Mismo patrón que `lib/harvestYear.ts` o `lib/grados/definicion.ts`: sin
// imports de React, sin `server-only` — lo consumen un componente cliente y un
// módulo de servidor por igual.
//
// SI EL OWNER CAMBIA UNA CIFRA, ESTE ES EL ARCHIVO. Y el cambio viaja solo a
// los datos estructurados.

import type { Lang } from "@/components/lang/i18n";

export type Faq = { q: string; lead: string; bullets: string[] };
...
export const FAQ: Record<Lang, FaqDict> = {
  es: {
